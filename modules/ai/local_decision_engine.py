...
def _handle_speech(data: dict) -> dict:
    ...
    command_patterns = [
        (r"(打开|开启|启动|开一下|开)空调|空调.*(打开|开启|启动|开)|^(太热|好热|有点热)$", "TurnOnAC", 0.92),
        (r"(关闭|关掉|关一下|关)空调|空调.*(关闭|关掉|关)|^(太冷|好冷|有点冷)$", "TurnOffAC", 0.92),
        (r"(调高|升高|提高).*温度|温度.*(调高|升高|提高)", "temp_up", 0.88),
        (r"(调低|降低|下降).*温度|温度.*(调低|降低|下降)", "temp_down", 0.88),
        (r"(播放|放一下|来一首|听一下|放).*(音乐|歌|歌曲|周杰伦|王菲|林俊杰|陈奕迅|邓紫棋|薛之谦)", "PlayMusic", 0.9),
        (r"^(播放|放一下|来一首|听一下|放)(音乐|歌)?$", "PlayMusic", 0.82),
        (r"(暂停|停止|停掉|关掉).*(音乐|播放|歌曲)|^(暂停播放|停止播放|关音乐|停音乐)$", "StopMusic", 0.9),
        ...
        (r"(打开|开一下|开).*车窗", "window_open", 0.9),
        (r"(关闭|关掉|关一下|关).*车窗", "window_close", 0.9),
        (r"(打开|开一下|开).*灯", "light_on", 0.86),
        (r"(关闭|关掉|关一下|关).*灯", "light_off", 0.86),
