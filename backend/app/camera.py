...
# ── 告警持久化（懒加载避免循环导入）──
_insert_alert_record = None
_get_session_id = None


def _ensure_db():
    global _insert_alert_record, _get_session_id
    if _insert_alert_record is None:
        from backend.app.core.database import insert_alert_record as _ins, get_current_session_id as _gsid
        _insert_alert_record = _ins
        _get_session_id = _gsid
...
def _loop(ws_manager):
    ...
    try:
        ...
        print(f"[camera] Gesture detector ready: {len(gesture_detector.labels)} gestures", flush=True)
    except Exception as e:
        print(f"[camera] Gesture not loaded: {e}", flush=True)
    ...
    gesture_name = ""
    gesture_conf = 0.0
    gesture_action_hold = ""    # 上一次手势动作
    gesture_action_until = 0.0  # 动作保持到的时间戳
    ...
    while _running:
        ...
        gesture_result = None
        if gesture_name:
            gresult = decide_locally({
                "trigger": "gesture",
                "data": {"state": gaze, "duration": 0,
                         "gesture": gesture_name, "confidence": gesture_conf, "text": ""}
            })
            print(f"[camera] decide_locally: gesture={gesture_name}, conf={gesture_conf:.2f}, result={gresult.get('action_code')}", flush=True)
            if gresult.get("action_code", "normal") != "unknown":
                gesture_result = gresult

        if gesture_result:
            safety_severity = result.get("severity", "normal")
            if safety_severity != "severe":
                result = gesture_result
                # 保持动作码至少 0.8 秒，让前端轮询能捕捉到
                gesture_action_hold = gesture_result.get("action_code", "")
                gesture_action_until = now + 0.8
        elif gesture_name:
            print(f"[camera] gesture detected but NO action mapped: {gesture_name}", flush=True)

        # ── 主动安全干预：severe/moderate 分心时 WS 广播 + 数据库持久化 ──
        severity = result.get("severity", "normal")
        cooldown_ok = (now - _last_safety_trigger.get("time", 0)) > _SAFETY_COOLDOWN_SEC
        if severity in ("severe", "moderate") and cooldown_ok:
            ...
            logger.info("主动安全告警: severity=%s", severity)
            # ── 持久化告警记录（写入 alerts 表）──
            try:
                _ensure_db()
                sid = _get_session_id() if _get_session_id else 0
                if sid > 0 and _insert_alert_record:
                    _insert_alert_record(
                        session_id=sid,
                        risk_level=severity,
                        alert_msg=f"[{category}] {label} — {tts_text}",
                        perclos=round(perclos_val, 3),
                        blink_rate=round(blink_rate_val, 1),
                        fatigue_score=float(fatigue_score),
                    )
            except Exception as e:
                logger.warning("告警持久化失败: %s", e)
        # 冷却重置：状态正常后允许下次触发
        if severity == "normal" and gaze == "center":
            _last_safety_trigger["time"] = 0

        # ── 手势识别（每 3 帧一次）──
        gesture_name = ""
        gesture_conf = 0.0
        if gesture_detector:
            hand_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            gname, gconf = gesture_detector.process(hand_rgb)
            if gname:
                gesture_name = gname
                gesture_conf = gconf
        ...
        with _lock:
            ...
            _frame = jpg.tobytes()
            stored_action = result.get("action_code", "normal")
            # 保持手势动作至少 0.8 秒，让前端轮询能捕捉到
            if stored_action == "normal" and gesture_action_hold and now < gesture_action_until:
                stored_action = gesture_action_hold
...
def start(ws):
    ...
    if os.getenv("CAMERA_ENABLED", "1") == "0":
        logger.info("CAMERA_ENABLED=0，跳过服务器端摄像头引擎（使用本地 app.py 推流）")
        return
    if _running: return
    _running = True
